...
void QLT::run(Session *session, const string &argument)
{
  ...
  if (argument.length())
  {
    ...
    if (session->loglevel >= 2)
      *(session->logfile) << "QLT :: Requested quality is " << factor << endl;
  }
}
...
void SDS::run(Session *session, const string &argument)
{
  ...
  if (session->loglevel >= 3)
    *(session->logfile) << "SDS handler reached" << endl;
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "SDS :: set to " << session->view->xangle << ", "
...
void MINMAX::run(Session *session, const string &argument)
{
  ...
  if (session->loglevel >= 3)
    *(session->logfile) << "MINMAX handler reached" << endl;
  ...
  if (nchan < 0 || nchan > (int)(*session->image)->getNumChannels())
  {
    if (session->loglevel >= 1)
      *(session->logfile) << "MINMAX :: Error: channel number out of bounds: "
                          << tmp.c_str() << endl;
    return;
  }
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "MINMAX :: set to " << (*(session->image))->min[nchan] << ", "
...
void CNT::run(Session *session, const string &argument)
{
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "CNT handler reached" << endl;
  ...
  if (arg == "eq" || arg == "equalization")
  {
    ...
    if (session->loglevel >= 3)
      *(session->logfile) << "CNT :: histogram equalization requested" << endl;
  }
  // Linear stretch
  else if (arg == "st" || arg == "stretch")
  {
    ...
    if (session->loglevel >= 3)
      *(session->logfile) << "CNT :: contrast stretch requested" << endl;
  }
  ...
  else
  {
    ...
    if (session->loglevel >= 3)
      *(session->logfile) << "CNT :: requested contrast adjustment is " << contrast << endl;
  }
}
...
void GAM::run(Session *session, const string &argument)
{
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "GAM handler reached" << endl;
  ...
  if (arg == "log" || arg == "logarithm")
  {
    ...
    if (session->loglevel >= 3)
      *(session->logfile) << "GAM :: log transform requested" << endl;
  }
  ...
  else
  {
    ...
    if (session->loglevel >= 3)
      *(session->logfile) << "GAM :: requested gamma adjustment is " << gamma << endl;
  }
}
...
void WID::run(Session *session, const string &argument)
{
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "WID handler reached" << endl;
  if (session->loglevel >= 3)
    *(session->logfile) << "WID :: requested width is " << requested_width << endl;
...
void HEI::run(Session *session, const string &argument)
{
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "HEI handler reached" << endl;
  if (session->loglevel >= 3)
    *(session->logfile) << "HEI :: requested height is " << requested_height << endl;
...
void RGN::run(Session *session, const string &argument)
{
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "RGN handler reached" << endl;
...
void ROT::run(Session *session, const string &argument)
{
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "ROT handler reached" << endl;
  if (session->loglevel >= 3)
    *(session->logfile) << "ROT :: requested rotation is " << rotation << " degrees" << endl;

  // Set rotation - watch for a '!180' request, which is simply a vertical flip
  if (session->view->flip == 1 && rotation == 180)
    session->view->flip = 2;
  else
...
void JTLS::run(Session *session, const string &argument)
{
  ...
  */
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "JTLS handler reached" << endl;
...
void SHD::run(Session *session, const string &argument)
{
  ...
  */
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "SHD handler reached" << endl;
  ...
  if (session->loglevel >= 3)
    *(session->logfile) << "SHD :: requested shade incidence angle is "
...
void CMP::run(Session *session, const string &argument)
{
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "CMP handler reached" << endl;
  if (session->loglevel >= 3)
    *(session->logfile) << "CMP :: requested colormap is " << ctype << endl;
  session->view->cmapped = true;

  if (ctype == "hot")
    session->view->cmap = HOT;
  else if (ctype == "cold")
    session->view->cmap = COLD;
  else if (ctype == "jet")
    session->view->cmap = JET;
  else if (ctype == "blue")
    session->view->cmap = BLUE;
  else if (ctype == "green")
    session->view->cmap = GREEN;
  else if (ctype == "red")
    session->view->cmap = RED;
  else
...
void INV::run(Session *session, const string &argument)
{
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "INV handler reached" << endl;
...
void LYR::run(Session *session, const string &argument)
{
  ...
  if (argument.length())
  {
    ...
    if (session->loglevel >= 2)
      *(session->logfile) << "LYR handler reached" << endl;
    if (session->loglevel >= 3)
      *(session->logfile) << "LYR :: requested layer is " << layer << endl;
...
void CTW::run(Session *session, const string &argument)
{
  ...
  */
  ...
  {
    if (session->loglevel >= 2)
      *(session->logfile) << "CTW handler reached" << endl;
  }
...
void COL::run(Session *session, const string &argument)
{
  ...
  */
  ...
  if (session->loglevel >= 2)
    *(session->logfile) << "COL handler reached" << endl;
  if (session->loglevel >= 3)
    *(session->logfile) << "COL :: requested color transform to " << ctype << endl;

  if (ctype == "grey" || ctype == "gray")
