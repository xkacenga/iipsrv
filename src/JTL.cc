...
void JTL::send(Session *session, int resolution, int tile)
{
  ...
  if (session->view->requireHistogram() && (*session->image)->histogram.size() == 0)
  {
    ...
    if (i != session->imageCache->end())
      (i->second).histogram = (*session->image)->histogram;
  }

  // Request uncompressed tile if raw pixel data is required for processing
  if ((*session->image)->getNumBitsPerPixel() > 8 || (*session->image)->getColourSpace() == CIELAB || (*session->image)->getNumChannels() == 2 || (*session->image)->getNumChannels() > 3 || ((session->view->colourspace == GREYSCALE || session->view->colourspace == BINARY) && (*session->image)->getNumChannels() == 3 && (*session->image)->getNumBitsPerPixel() == 8) || session->view->floatProcessing() || session->view->equalization || session->view->getRotation() != 0.0 || session->view->flip != 0)
    ct = UNCOMPRESSED;
  ...
  if (rawtile.bpc > 8 || session->view->floatProcessing())
  {
    ...
    if (session->view->contrast == -1)
    {
      ...
      unsigned int n0 = 0;
      while ((*session->image)->histogram[n0] == 0)
        ++n0;

      // Find highest bin
      unsigned int n1 = (*session->image)->histogram.size() - 1;
      while ((*session->image)->histogram[n1] == 0)
        --n1;
    ...
    if (session->view->gamma != 1.0)
    {
      ...
      {
        if (gamma == -1)
          *(session->logfile) << "JTL :: Applying logarithm transform in ";
        else
          *(session->logfile) << "JTL :: Applying gamma of " << gamma << " in ";
  ...
  if (session->view->flip != 0)
  {
    ...
    if (session->loglevel >= 5)
    {
      *(session->logfile) << "JTL :: Flipping image ";
      if (session->view->flip == 1)
        *(session->logfile) << "horizontally";
      else
        *(session->logfile) << "vertically";
